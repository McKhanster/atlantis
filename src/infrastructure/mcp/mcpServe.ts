...
import { SessionData } from '../session-data/index.js';

// Check for OAuth flag
const session = SessionData.getInstance();
...
export class AtlantisMcpServer {
    ...
    // Start HTTP server
    async startHttp(port = MCP_PORT): Promise<void> {

        this.app.use(express.json());
        this.app.post('/mcp', mcpPostHandler);
        this.app.get('/mcp', mcpGetHandler);
        this.app.delete('/mcp', mcpDeleteHandler);
...
const mcpPostHandler = async (req: Request, res: Response) => {
    ...
    if (sessionId) {
        console.log(`Received MCP request for session: `, req);
    } else {
    ...
    try {
        if (sessionId) {
            console.log("Name: ", session.getSession(sessionId))
        }
            ...
            transport = new StreamableHTTPServerTransport({
                ...
                onsessioninitialized: sessionId => {
                    ...
                    console.log(`Session initialized with ID: ${sessionId}`);
                     session.setSession(sessionId, req.body.params.clientInfo.name);
...
process.on('SIGINT', async () => {
    ...
    // Close all active transports to properly clean up resource
